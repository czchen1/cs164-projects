...
class Type_Var_AST : public Type {
    ...
    void codeGen (Code_Context& context, bool constructor) override {
        // The type of a type variable is Any. Right?
        context << "Any";
    }
...
protected:
    ...
    void codeGen (Code_Context& context, bool constructor) override {
        string name = child (0)->text ();
        unordered_map<string, string>::const_iterator entryPair = builtInNames.find (name);
        if (entryPair != builtInNames.end ()) {
            context << entryPair->second;
        } else {
            context << child(0)->uniqueName();
        }
        if (arity () > 1 && (name.compare("dict") != 0) && (name.compare("list") != 0)) {
            context << "<";
