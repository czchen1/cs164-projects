...
public:
    ...
    virtual void codeGenOuter (Code_Context& context);

    virtual std::string uniqueName ();
    virtual void codeGenEnvStructs (Code_Context& context, std::string parent_function="global");

    /** Generates code for function, variable, and class definitions -- these 
     * reside outside of the main function **/
    virtual void codeGenDecls (Code_Context& context);

    /** Helper function for codeGenDecls. Forward declares all variables. **/
    virtual void codeGenForwardVarDecl (Code_Context& context, Decl* current);

    /** Generates code within main function **/
    virtual void codeGen (Code_Context& context, bool constructor=false);

    /** Generates comma separated code for my children, beginning
     * with child of index "start" and ending with child of index "stop". */
    virtual void commaSeparatedCodeGen (Code_Context& context, int start, int stop=-1, bool codeGenDecls=false);
