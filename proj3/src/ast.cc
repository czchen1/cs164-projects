...
void
AST::codeGenOuter (Code_Context& context)
{
    //codeGenEnvStructs (context);
    codeGenDecls (context);
    codeGen (context);
}

void
AST::codeGenEnvStructs (Code_Context& context, string parent_function)
{
    for (auto c : *this) {
        c->codeGenEnvStructs (context);
    }
}

string AST::uniqueName () {
    return getId ()->text () + "_" + to_string(getDecl ()->getIndex ());
}

void
AST::codeGenDecls (Code_Context& context)
{
    for (auto c : *this) {
...
void
AST::codeGenForwardVarDecl (Code_Context& context, Decl* current)
{
    for (Decl* d : current->getEnviron()->get_members()) {
        if (d->assignable ()) {
            d->getType()->codeGen(context);
            context << " ";
            context << d->getName () << "_" << to_string(d->getIndex ());
            context << ";" << endl;
        }
        else if (d->isFunc())
            {
...
void
AST::codeGen (Code_Context& context, bool constructor)
{
    for (auto c : *this) {
...
void
AST::commaSeparatedCodeGen (Code_Context& context, int start, int stop, bool codeGenDecls) {
    assert (start >= 0 && stop <= arity ());
    if (arity () <= start)
        return;
    codeGenDecls ? child (start)->codeGenDecls (context) : child (start)->codeGen (context);
    if (arity() > 1)
    {
        for (auto c: trimmedAST (*this, start+1, stop)) {
            context << ", ";
            codeGenDecls ? c->codeGenDecls (context) : c->codeGen (context);
        }
    }
}
