...
public:
    ...
    /** Returns my (first) Decl, or NULL if there is none yet
     *  assigned. */
    ...
    /** Add all declarations reachable from me to DECLS. */
    virtual void reachableDecls (Decl_Set& decls);
    ...
    // Added, basically the same thing as outer but maintaing a binding
    virtual void resolveTypesInner(Decl* context, Unifier& bind);


    virtual void resolveDeclTypes(Unifier& bind);
...
private:
    ...
    /** My syntactic index. */
    AST_Index index;
    /** My type, or NULL if I have none or it has not been determined. */
    Type_Ptr _type;
};
...
protected:
    ...
    void print (AST_Printer& printer);
...
public:
     ...
     *  returns a special dedicated ID. NULL for unbound type
